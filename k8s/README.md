# sm-data-exchange Kubernetes 部署
...
2. 密钥 `sm-secrets` 已创建（字段：`internal_api_key`、`sm4_key_hex`）：
   ...
   ```
3. 数据卷 PVC `sm-data-exchange-data`（按存储环境预创建，或改用 emptyDir）。
...
- `service.yaml`：ClusterIP 服务（端口 8480）。
- `networkpolicy.yaml`：默认拒绝（Deny-all），仅允许同命名空间 SM 服务与 API 网关访问，出站仅限审计中心(8320)/通知中心(8470)/DNS(53)。

## 说明

- 镜像 `ghcr.io/luoshitianchen/sm-data-exchange:latest` 按实际发布仓库与 tag 替换。
